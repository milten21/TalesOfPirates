...
namespace Top.Client.Assets.Maps.Tests
{
    public class MapStoreTests
    {
        ...
        private static MemoryContent Content()
        {
            ...
            {
                content.Add(MapTexturePaths.Water(frame), Png(frame == 3 ? 4 : 2, Blue));
            }

            return content;
        }

        private static MapStore Store(MemoryContent content)
        {
            return new MapStore(content, Terrains);
        }
