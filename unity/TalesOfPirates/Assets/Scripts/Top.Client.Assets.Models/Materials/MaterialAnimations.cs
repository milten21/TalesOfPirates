...
{
    /// <summary>
    /// The MaterialAnimations class is responsible for managing and applying
    /// material-based animations (e.g., UV transformations, opacity changes,
    /// and texture image animations) to renderers in a scene.
    /// </summary>
    public class MaterialAnimations
    {
        ...
        public void Attach(Renderer renderer, int[] materialIndices)
        {
            for (var slot = 0; slot < materialIndices.Length; slot++)
            {
                ...
                if (tracks.Uv != null)
                {
                    var animation = renderer.gameObject.AddComponent<UvMatrixAnimation>();
                ...
                {
                    var animation = renderer.gameObject.AddComponent<TextureImageAnimation>();
        ...
        private Tracks Build(MaterialExtras extras)
        {
            ...
            return new Tracks
            {
                ...
                Flipbook = extras.Flipbook != null
                    ? Kept(TrackMapper.CreateTextureImageTrack(extras.Flipbook, _gltf.GetTexture))
                    : null,
            };
        }
